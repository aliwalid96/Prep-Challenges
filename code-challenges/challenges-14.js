...
// Resource:
//l split: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/split
...
const replaceWords = (str) => {
    ...
    for(let i=0;i<splitted.length;i++){
        if(splitted[i]==="I"){
            let newa= splitted.splice(i, 1, "we");
            newa.join("");
        } 
        if(splitted[i]==="amm"){
           let newa= splitted.splice(i, 1, "are");
           newa.join("");
        } 
        if(splitted[i]==="was") {
            let newa= splitted.splice(i, 1, "were");
            newa.join("");
        } 

    }
    // splitted.array.forEach(element => {
    //     if(element.splitted[i]==="I") return splitted.splice(i, 1, "we")
    //     if(element.splitted[i]==="amm") return splitted.splice(i, 1, "are")
    //     if(element.splitted[i]==="was") return splitted.splice(i, 1, "were")
